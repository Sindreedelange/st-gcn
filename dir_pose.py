...
def run_pose_estimation(input_path, output_path):
    ...
    for file in os.listdir(input_path):
        ...
        output_fpath = os.path.join(output_path, file)

        if check_duplicates(output_path, file):
            print("\n Duplicate file: {}\n".format(output_fpath))
            continue

        print("\n File {} does not exist".format(output_fpath))
...
if __name__ == '__main__':
    ...
    data_path = '../from_server/data'
    #youtube_path = os.path.join(data_path, 'youtube')
    kinetics_path = os.path.join(data_path, 'Kinetics/kinetics-skeleton/')

    videos_clean_path = os.path.join(data_path, 'videos_clean_{}'.format(phase))
    videos_keypoints_path = os.path.join(data_path, 'videos_clean_keypoints_{}'.format(phase))
